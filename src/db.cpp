...
void db::vox_data_delete(int x, int y, int z)
{
  ...
  std::vector<uchar> VoxData = load_vox_data(x, z); // Загрузить "колонку" воксов из БД
...
void db::vox_data_save (vox* pV)
{
  ...
  std::vector<uchar> VoxData = load_vox_data(pV->Origin.x, pV->Origin.z);
...
/// \return
///
std::vector<uchar> db::load_vox_data(int x, int z)
{
  char query[127] = {0};
