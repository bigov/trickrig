...
namespace tr
{
    ...
    for(int x = xMin; x<= xMax; x += g1)
      for(int y = yMin; y<= yMax; y += g1)
        for(int z = zMin; z<= zMax; z += g1)
          RigsDb0.place_in_gpu(RigsDb0.get({x, y, z}));
  ...
  void space::redraw_borders_x()
  {
    ...
    for(int y = yMin; y <= yMax; y += g1)
      for(int z = zMin; z <= zMax; z += g1)
        RigsDb0.remove_from_gpu(RigsDb0.get({x_old, y, z}));
    ...
    for(int y = yMin; y <= yMax; y += g1)
      for(int z = zMin; z <= zMax; z += g1)
        RigsDb0.place_in_gpu(RigsDb0.get({x_new, y, z}));
  ...
  void space::redraw_borders_z()
  {
    ...
    for(int y = yMin; y <= yMax; y += g1)
      for(int x = xMin; x <= xMax; x += g1)
        RigsDb0.remove_from_gpu(RigsDb0.get({x, y, z_old}));
    ...
    for(int y = yMin; y <= yMax; y += g1)
      for(int x = xMin; x <= xMax; x += g1)
        RigsDb0.place_in_gpu(RigsDb0.get({x, y, z_new}));
