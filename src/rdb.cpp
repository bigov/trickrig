...
void rdb::decrease(int i)
{
  ...
  rig_wipe(B);                               // убрать из рендера выбранный риг
  MapRigs.erase(B->Origin);                  // удалить риг из базы данных
...
void rdb::rig_draw(voxel* R)
{
  ...
  {
    if(!R->side_fill_data(side_id, buffer, R->Origin)) continue;
    vbo_addr = VBO->append(buffer, bytes_per_snip);
    R->offset_write(side_id, vbo_addr);             // записать в бокс адрес смещения VBO
    Visible[vbo_addr] = R;                        // добавить ссылку на бокс
...
void rdb::rig_wipe(voxel* Rig)
{
  ...
  for(u_char side_id = 0; side_id < SIDES_COUNT; ++side_id)
  {
    GLsizeiptr dest = Rig->offset_read(side_id);  // адрес данных, которые будут перезаписаны
    if(dest < 0) continue;                     // -1 если сторона невидима - пропустить цикл
    Rig->offset_write(side_id, -1);
    ...
    {                                     // место переписываются данные с адреса free.
      voxel* BMoved = Visible[free];        // Адрес бокса, чьи данные переносятся.
...
///
voxel* rdb::gen_rig(const i3d& P, u_char, u_char, u_char)
{
  voxel* R = get(P);            // Если в этой точке уже есть риг,
...
///
void rdb::visibility_recalc_rigs(voxel* R0)
{
  ...
  {                                                  // получить
    R1 = get(i3d_shift(R0->Origin, side, lod));      // ссылку на соседний риг.
    if(nullptr == R1) continue;                      // Если пусто - пропустить.
    R0->visible_check(side, R1);                     // Проверить стыковку (видимость) стороны
...
void rdb::visibility_recalc(i3d P0)
{
  ...
  for (u_char side = 0; side < SIDES_COUNT; ++side)
  {
    ...
    Rig->visible[opposite(side)] = true;
  }
}
